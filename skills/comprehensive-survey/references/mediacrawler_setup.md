...
MediaCrawler 是社交平台爬取工具，支持 B站等平台的内容搜索和采集。
...
B站等平台需要登录态才能搜索。MediaCrawler 通过 Cookie 模拟已登录状态。
...
mkdir -p $DATA_BASE/{bili/{json,audio},transcripts}
```
...
# 测试搜索（需要有效 Cookie）
python main.py --platform bili --lt cookie --type search --keywords "测试" --count 1
```
